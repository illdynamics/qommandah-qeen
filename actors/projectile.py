...
class Projectile(Entity):
    ...
    def update(self, dt: float) -> None:
        ...
        self.set_position(self.position.x + movement.x, self.position.y + movement.y)
    ...
    def check_collision(self, collision_system: CollisionSystem) -> List[CollisionResult]:
        ...
        if not self.active:
            return []
        return collision_system.check_tile_collision(self.get_rect())
    ...
    def check_entity_collision(self, entities: List[Entity]) -> Optional[Entity]:
        ...
        for entity in entities:
            if entity == self.owner or entity in self.hit_entities:
                continue
            if entity.is_active() and projectile_rect.colliderect(entity.get_rect()):
                return entity
        return None
